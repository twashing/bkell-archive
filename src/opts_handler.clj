...
(defn option_handler [node handler] 
    (try 
                ...
                (if (instance? com.interrupt.bookkeeping.cc.node.AOptsCommandInput (. node getCommandInput) )
					(do 
					   (clojure.contrib.logging/info (str "DEBUG > OPTIONS input > token[" (.. node getCommandInput getInputOption getCommandtoken) "] > options[" (.. node getCommandInput getInputOption getCommandoption) "]"))
					   ...
					   (def option-id  
						...
						(clojure.contrib.logging/info (str "Hello World >> " (.. (nth option-id 0) getIdOpt getText) " >> " (type (.. (nth option-id 0) getIdOpt getText)) ))
					   ...
					   (clojure.contrib.logging/info (str "DEBUG > extracted > [" token "] > [" options "] > [" db-id-ID "]"))
					   ...
					   (clojure.contrib.logging/info (str "DEBUG > db-base-URL["db-base-URL"] > db-system-DIR["db-system-DIR"] > db-working-DIR["db-working-DIR"] > leaf["db-leaf"]") )
					   (clojure.contrib.logging/info (str "DEBUG > db-base-URL[" db-full-PARENT "]"))
